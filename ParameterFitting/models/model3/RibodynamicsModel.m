function dx = RibodynamicsModel(t,x, theta,dataset)
    ...
    dx(1) = copies*a_tet/fAtc - mu*x(1) - delta_s*x(1) - k_on*x(1)*x(2) + k_off*x(3) + delta_m*(x(3) + x(4)) ; %sRNA

    dx(2) = copies*a_lac/fIptg - mu*x(2) - delta_m*x(2) - k_on*x(1)*x(2) + k_off*x(3) + delta_s*(x(3) + x(4)) ; %mRNA

    dx(3) = k_on*x(1)*x(2) - k_off*x(3) - k_hyb*x(3) - mu*x(3) - (delta_m + delta_s)*x(3); %sRNA:mRNA_intermediate

    dx(4) = k_hyb*x(3) - mu*x(4) - (delta_m  + delta_s + delta_c)*x(4); %sRNA:mRNA_stable
