function dx = RibodynamicsModel(t,x, theta,dataset)
    ...
    dx(6) = beta*x(3) + f_srna*beta*x(5) - matur*x(6) - mu*x(6) - delta_g*x(6) - (vz*x(6))/(Kz + x(6) + c*(x(7)-z0)); %GFP non-mature

    dx(7) = (1/c)* ( matur*x(6) - (mu + delta_g)*c*(x(7)-z0) - ((vz*c*(x(7)-z0))/(Kz + x(6) + c*(x(7)-z0))) ); %measured fluoresence
